...
//!
/***
 这段代码是一个模板类GenericBuffer的定义，用于处理设备和主机上的缓冲区的分配，释放和查询。
 它是一个RAII（Resource Acquisition Is Initialization）类，即在构造函数中获取资源，
 在析构函数中释放资源。该类模板有两个模板参数：AllocFunc和FreeFunc，用于分配和释放缓冲
 区的操作。AllocFunc必须是一个函数对象，接受参数(void** ptr, size_t size)，并返回bool
 值。ptr是指向应存储分配的缓冲区地址的指针，size是要分配的内存大小（以字节为单位）。布尔值
 表示内存分配是否成功。FreeFunc必须是一个函数对象，接受参数(void* ptr)，并返回void。ptr
 是分配的缓冲区地址。它必须能够处理nullptr输入。
    GenericBuffer类具有以下成员函数和成员变量：
    构造函数：有多个重载形式，用于构造一个空的缓冲区或指定分配大小的缓冲区。
    移动构造函数和移动赋值运算符：支持对缓冲区进行移动操作。
    data()函数：返回底层数组的指针。
    size()函数：返回缓冲区的大小（元素数量）。
    nbBytes()函数：返回缓冲区的大小（字节数）。
    resize()函数：重新调整缓冲区的大小。如果新大小小于或等于当前容量，则不执行任何操作。
    ~GenericBuffer()析构函数：释放分配的缓冲区。
    在该类中，mSize表示缓冲区的大小（元素数量），mCapacity表示缓冲区的容量，mType表示
    缓冲区中的数据类型，mBuffer是指向分配的缓冲区的指针。allocFn和freeFn是分配和释放缓冲区的函数对象。
该类的主要目的是通过模板化的方式提供一个通用的缓冲区处理类，可以处理任意类型的数据，通过提供自定义的分配
和释放函数，使得可以在设备和主机上灵活地管理缓冲区。
*/
...
//! 这个RAII类处理主机和设备缓冲区的分配和回收，主机和设备缓冲区之间的内存来帮助推理，
//！调试转储来验证推理。BufferManager
//！类旨在用于简化缓冲区管理以及缓冲区与引擎之间的任何交互。
class BufferManager
{
public:
