...
/**
 * 
 * 该结构体可以用于测量代码块的运行时间，通过调用 Tic() 开始计时，
 * 然后调用 Toc() 结束计时并获取时间间隔。timeBuff 向量可以用于
 * 记录多次运行的时间间隔，可以通过调用 aveCost() 获取平均时间，
 * devCost() 获取时间的标准差。
*/
struct TicToc
{
...
vector<string> GetPngFiles(string strPngDir)
{
    ...
    int n = scandir(strPngDir.c_str(), &namelist, [](const struct dirent *cur) -> int {
        ...
        // return str.find(".jpg") != std::string::npos; }, alphasort);
    ...
    std::vector<cv::DMatch> &inlierMatches, std::vector<cv::DMatch> &wrongMatches)
{
    ...
    cv::Mat inliers;
    // E矩阵的自由度是5 旋转和平移每个3个自由度总共6个 因为尺度等价性 减去1 总共有五个尺度
