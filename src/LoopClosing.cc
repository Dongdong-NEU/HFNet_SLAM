...
bool LoopClosing::NewDetectCommonRegions()
{
    ...
    // 如果关闭回环检测 标志位：mbActiveLC。将不执行循环闭合和合并操作。
    if(!mbActiveLC)
        return false;
    ...
    //Check the last candidates with geometric validation （几何验证）

    // 是否检测到回环的标志位
    bool bLoopDetectedInKF = false;
    bool bCheckSpatial = false; // Spatial（空间的）
    ...
    if(mnLoopNumCoincidences > 0)
    {
        ...
        // mTcw * mTwc 当前帧 到 上一帧 图像的变换？
        Sophus::SE3d mTcl = (mpCurrentKF->GetPose() * mpLoopLastCurrentKF->GetPoseInverse()).cast<double>();
        // 使用构造函数初始化，传入参数为一个四元数（旋转部分）、平移矢量和尺度因子
        // 使用这个单位四元数、平移矢量和尺度因子（这里设为 1.0），创建了一个 g2o::Sim3 对象 gScl，表示尺度不变的相似变换。
        ...
        // 匹配当前帧到上一帧mpLoopMatchedKF，通过Sim3优化后看是否匹配点大于100个
        bool bCommonRegion = DetectAndReffineSim3FromLastKF(mpCurrentKF, mpLoopMatchedKF, 
                                         gScw, numProjMatches, mvpLoopMPs, vpMatchedMPs);
        // 检测到当前帧到上一帧的匹配点足够
        if(bCommonRegion)
        {
            // 检测到一个回环帧
...
#endif
    // mbMergeDetected  >= 3 或者 mbLoopDetected >= 3 返回true;
    ...
    { 
        mbLoopDetected = DetectCommonRegionsFromBoW(vpLoopBowCand, // 通过遍历搜索到的候选帧
                                                    mpLoopMatchedKF, // 最后成功匹配的候选关键帧
                                                    mpLoopLastCurrentKF, // 用于记录当前关键帧为上
                                                                         // 一个关键帧(后续若仍需要时序几何校验需要记录此信息)
                                                    mg2oLoopSlw, // 候选关键帧世界坐标系到当前关键帧的Sim3变换
                                                    mnLoopNumCoincidences,  // 成功几何验证的帧数，超过3就认为几何验证成功，不超过继续进行时序验证
                                                    mvpLoopMPs,  // 所有地图点
                                                    mvpLoopMatchedMPs);// 成功匹配的地图点 
    }
    // Merge candidates
    if(!bMergeDetectedInKF && !vpMergeBowCand.empty())
    {
        mbMergeDetected = DetectCommonRegionsFromBoW(vpMergeBowCand, mpMergeMatchedKF, 
                mpMergeLastCurrentKF, mg2oMergeSlw, mnMergeNumCoincidences, mvpMergeMPs, 
                mvpMergeMatchedMPs);
    }
...
bool LoopClosing::DetectAndReffineSim3FromLastKF(KeyFrame* pCurrentKF, KeyFrame* pMatchedKF, 
        g2o::Sim3 &gScw, int &nNumProjMatches,std::vector<MapPoint*> &vpMPs, 
        std::vector<MapPoint*> &vpMatchedMPs)
{
    set<MapPoint*> spAlreadyMatchedMPs;
    // 通过找出pMatchedKF的共试图中的地图点投影到pCurrentKF中去找到更多的匹配
    nNumProjMatches = FindMatchesByProjection(pCurrentKF, pMatchedKF, 
                                             gScw, 
                                             spAlreadyMatchedMPs, 
                                             vpMPs, 
                                             vpMatchedMPs);
    ...
    if(nNumProjMatches >= nProjMatches)
    {
        ...
        if(numOptMatches > nProjOptMatches)
        {
            ...
            nNumProjMatches = FindMatchesByProjection(pCurrentKF, pMatchedKF, 
                                                      gScw_estimation, spAlreadyMatchedMPs,
                                                      vpMPs, vpMatchedMPs);
            // 通过sim3变换优化 匹配点是否大于100个
                                             ...
                                             std::vector<MapPoint*> &vpMatchedMPs)
{
    ...
    for(KeyFrame* pKFi : vpBowCand)
    {
        ...
        for(int j=0; j<vpCovKFi.size(); ++j)
        {
            ...
            {
                // i是当前候选帧，j是候选帧公式组内第几个帧
                MapPoint* pMPi_j = vvpMatchedMPs[j][k];
                if(!pMPi_j || pMPi_j->isBad())
                    continue;
                // 将本次候选帧组与当前关键帧匹配上的所有地图点保存到一个容器中
        ...
        if(numBoWMatches >= nBoWMatches) // TODO pick a good threshold 
        {
            ...
            if(mpTracker->mSensor==System::IMU_MONOCULAR && !mpCurrentKF->GetMap()->GetIniertialBA2())
                bFixedScale=false;
            // 传入了 当前帧 当前候选帧 当前帧和当前候选帧组匹配上的点 是否固定尺度 匹配上的点对应的帧
            ...
            if(bConverge)
            {
                ...
                // 清空当前候选帧的共视组，放为当前共视组中匹配数量最多的一帧的前10个共视帧
                vpCovKFi.clear();
                ...
                // 将新候选帧组的地图点投影到当前帧中，看匹配上了多少点
                int numProjMatches = matcher.SearchByProjection(mpCurrentKF, 
                                                                mScw, 
                                                                vpMapPoints, // 新的候选帧组地图点
                                                                vpKeyFrames, // 新的候选帧组地图点对应的关键帧
                                                                vpMatchedMP, // 投影匹配后放置的地图点
                                                                vpMatchedKF, // 投影匹配后放置的关键帧
                                                                12, 
                                            (matcher.TH_HIGH + matcher.TH_LOW) / 2.0);
                ...
                
                {
                    ...
                    // nSim3Inliers = 35; nProjMatches = 50;
                    ...
                    if(numOptMatches >= nSim3Inliers)
                    {
                        ...
                        if(numProjOptMatches >= nProjOptMatches)
                        {
                            ...
                            // 选取当前关键帧最好的10帧共视图来验证有几个和当前最佳候选帧能匹配关系达标，是否有30个点匹配的上
                            ...
                            {
                                // numProjOptMatches：优化过后的sim3投影匹配点数
                                nBestMatchesReproj = numProjOptMatches;
                                // nNumKFs新候选帧组中有几帧通过验证
                                        ...
                                        vector<MapPoint*> &vpMatchedMapPoints)
{
    ...
    if(nInitialCov < nNumCovisibles)
    {
        for(int i=0; i<nInitialCov; ++i)
        {
            ...
            {   // 如果这些二级共试图没有在这两帧共试图的容器中，插入loopKF共试图容器中
