...
{  
    ...
    for (seq = 0; seq<num_seq; seq++)
    {
        ...
        //  输出：图像名称 vector<vector<str>> seq是第几个序列的意思；
        //  输出：时间戳 vector<vector<double>> seq是第几个序列的意思；              
        vstrImageFilenames[seq], vTimestampsCam[seq]);
        cout << "LOADED!" << endl;
        // nImages中存储这每一个序列有多少图像；
    ...
    // 创建SLAM系统；        相机参数配置文件 相机模式                       是否开启可视化
    ...
    for (seq = 0; seq<num_seq; seq++)
    {
        ...
        for(int ni=0; ni<nImages[seq]; ni++, proccIm++)
        {
            ...
            // Read image from file 读取图像与图像对应的时间戳
            ...
            if(ttrack<T) {
                // 如果这当前帧和上一帧追踪时间太短，比两帧图像采集时间间隔还短的话，延时；
