                         ...
                         std::vector<cv::DMatch>& vMatches)
{
    ...
    // 这里应该用L2距离来评估距离的 不该这样作 但是也差不多吧 两个向量为平行的时候cos为1，1 - 1 = 0 ， 2* 0为0，距离为0
...
int main(int argc, char* argv[])
{
    ...
    const string image1_path = string(argv[1]);
    const string image2_path = string(argv[2]);
    const string strModelPath = string(argv[3]);
    const double img_hight = std::stod(argv[4]);
    const double img_width = std::stod(argv[5]);
    ...
    cv::resize(image1, image1, cv::Size(img_width, img_hight));
    cv::resize(image2, image2, cv::Size(img_width, img_hight));
    cv::Size ImSize = image1.size();
    ...
    do {
        ...
        cout << "-------------------------------------------------------" << endl;
        {
            ...
            cv::Mat E = FindCorrectMatchesByEssentialMat(
                keypointsHF1, keypointsHF2, thresholdMatchesHF, cv::Mat::eye(3,3, CV_32F), 
                inlierMatchesHF, wrongMatchesHF);

            cv::Mat plotHF = ShowCorrectMatches(
                image1, image2, keypointsHF1, keypointsHF2, inlierMatchesHF, wrongMatchesHF,
                 showWholeMatches);
        ...
        }
        {
            ...
            // 用基础矩阵去筛选正确匹配点（RANSAC）
            cv::Mat E = FindCorrectMatchesByEssentialMat(keypointsHF1, keypointsHF2, 
            thresholdMatchesHF, cv::Mat::eye(3,3, CV_32F), inlierMatchesHF, wrongMatchesHF);
            // 显示匹配关系
            cv::Mat plotHF = ShowCorrectMatches(image1, image2, keypointsHF1, keypointsHF2, 
            inlierMatchesHF, wrongMatchesHF, showWholeMatches);
